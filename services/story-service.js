...
var storyService = {
  ...
  upsert: function(obj, callback){
    ...
    if(obj.id !== null && obj.id !== undefined && obj.id !== ""){
     ...
     storySchema.findOneAndUpdate({_id: id}, {content:obj.content, type: obj.type}, {new: true, upsert: true}, function(error, doc){
       callback();
       /**
   ...
   */
  delete: function(id){
    storySchema.remove({ _id : id }, function (err) {
  ...
  getOne: function(id, callback){
    storySchema.findOne({_id: id}, function (err, story) {
      if (err) console.error(err);
        callback(story);
        /**
         * @callback requestCallback
  ...
  generate: function(callback){
    ...
    storySchema.find(function (err, storiesCollection) {
       ...
       */
        var storyData = {
          begining: storiesCollection.filter(function(obj){ return condition(obj, 1);}),
          middle: storiesCollection.filter(function(obj){ return condition(obj, 2);}),
          end: storiesCollection.filter(function(obj){ return condition(obj, 3);})
        }
